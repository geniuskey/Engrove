...
docker compose --env-file /etc/engrove/production.env \
  ...
  -f deploy/compose/compose.production.yaml config --quiet
```
...
Create and verify an encrypted backup first. Pull the reviewed release, inspect new migrations and release notes, build images, run the one-shot `migrate` service, then replace application services. Application startup never performs schema push. Check readiness, `/metrics`, a sign-in, and an exact dataset/chart source read after each upgrade.
