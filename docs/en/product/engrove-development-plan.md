...
> **Status:** Draft v0.10<br>
> **Product:** Engrove<br>
> **Category:** Self-hosted Engineering Data & Operations Workspace<br>
> **Primary audience:** Manufacturing R&D, test, validation, reliability, and engineering teams<br>
> **Primary deployment:** Self-hosted / on-premise<br>
> **Implementation style:** Modular monolith with background workers<br>
> **Last decision update:** 2026-08-11
...
30. Named views are `RecordView` entities with explicit collaborative, personal, or locked edit modes. A view persists visible-field order, field widths, filters, sorts, row density, page size, ownership, and an optional lock note with optimistic concurrency and audit history. `schema.read` may list and use views; `view.manage` lets Contributors create and edit collaborative views plus their own personal views. Only the collaborative-view author may convert it to personal, while Engineers and administrators may change any mode, including lock/unlock. A locked view must be unlocked before its saved configuration changes. The built-in `All records` view is virtual and cannot be modified or archived.
...
33. Engineers and administrators may publish a persisted view as a revocable public link. Grid, Gallery, Kanban, and Calendar links are read-only, include only visible fields and the saved record scope, support optional password protection, expiry and CSV download, and permit non-persistent visible-field exploration. Form links are write-only intake surfaces: only public-safe visible fields are accepted, every request is rate-limited and idempotent, bot-trapped, validated through the normal record transaction, recorded in immutable submission provenance, audited with no actor, and emitted to webhooks with an explicit `public_form` source. Anonymous input is never attributed to the link owner. Share and unlock tokens are stored only as digests; the full share URL is shown once.
34. Relation fields are selected by searchable record display names rather than pasted UUIDs. Record reads hydrate linked display metadata in one bounded batch per page, while the candidate endpoint returns at most 100 records and excludes archived targets from new choices. Existing archived links remain resolvable and visibly marked so history is understandable without weakening stable-ID mutations.
35. Configurable records provide bounded lightweight comments in the record drawer and full detail page. Contributors and Reviewers may participate without record-edit permission, only authors may edit with optimistic concurrency, and every edit preserves the previous body in the record audit trail. Archived-record discussions remain readable but immutable.
36. Record comments store explicit active-member mentions and deliver preference-aware notifications to newly mentioned users. Notifications use a database-enforced task-or-record target, deep-link to the exact record comment, and never re-alert unchanged mentions after an edit.
37. Production installation and upgrades use a non-secret-printing preflight CLI that parses rather than executes the operator environment file, enforces mode `0600`, HTTPS and explicit proxy trust, distinct strong credentials, safe OIDC provisioning, and age backup configuration, then validates rendered Compose port isolation, runtime hardening, database roles, and Python-worker credential isolation.
38. Grid field summaries are persisted per named view and computed by PostgreSQL over the complete filtered record scope, independently of page size. Decimal results remain strings, quantity and latest-current-measurement summaries declare their canonical unit, and incompatible field/operation pairs fail validation rather than silently falling back to a visible-page calculation.
39. Grid views support up to three ordered scalar-field grouping levels. Each level persists its direction and enabled state, disabled levels retain their configuration, and PostgreSQL returns full-filter hierarchy counts independently of record pagination. Group keys lead user-defined sorts, nulls sort last, and unsupported multi-value or derived fields fail validation rather than duplicating records across paths. The existing single-group Kanban contract remains compatible.
40. A grouped Grid query applies its existing field-summary selection to every hierarchy node as well as the global footer. PostgreSQL computes count, sum, average, minimum, and maximum values over one materialized filtered-record scope, including canonical-unit latest-current-measurement results and record-distinct relation counts. The UI shows compact accessible summary badges in each group header, avoiding a second group-only summary configuration and any visible-page calculation.
41. Project task flow insights are derived from immutable status and lifecycle history rather than manually entered progress. The project-wide report exposes current WIP and status age, configurable stale-work signals, completed throughput, median and 85th-percentile cycle time, the five oldest active tasks, and a bounded completed-work distribution. It reconstructs daily end-of-day counts across current and archived workflow statuses as an accessible cumulative-flow chart, excluding task archive intervals and resuming counts after restoration, and pairs it with a daily created-versus-currently-completed trend. Created minus completed is labelled as intake pressure rather than exact backlog change because reopen and archive lifecycle events are distinct. Cycle time sums intervals in workflow statuses categorized as `in_progress`, including reopened work, while the browser presents the report as a non-blocking inline panel with direct task drill-downs.
42. Production HTTP delivery treats browser assets and programmable API representations differently. Fingerprinted web assets are immutable for one year, HTML is revalidated, missing assets never receive the SPA shell, and the origin emits CSP, frame, MIME-sniffing, referrer, and permissions defenses. Authenticated versioned API reads are private and revalidated with CORS-visible ETags, while secret-bearing share operations remain `no-store`; conditional requests still pass authorization and quota enforcement.
43. Browser authentication failure is handled as one application-level state transition rather than repeated page-local request errors. An authoritative `401 AUTHENTICATION_REQUIRED` unmounts protected content, shows an accessible session-expiry explanation, and preserves the exact bounded internal return path for local or OIDC reauthentication. OIDC seals that path into its short-lived state cookie and rejects external, protocol-relative, oversized, backslash-containing, and authentication-loop destinations.
44. An unexpected protected-page render failure is contained by a bilingual application boundary instead of producing a blank screen. Ordinary component failures retain the route and offer an in-place remount; lazy-chunk failures identify the stale/degraded delivery case and make a full reload the primary recovery. The browser sends a best-effort, session-only operational report containing a user-visible UUID, bounded internal pathname, stable error kind/name, and React component stack. The strict API excludes messages, query strings, fragments, user-entered values, full URLs, JavaScript error stacks, and personal API-token access; accepted reports increment a dedicated Prometheus counter.
45. Task duplication is a reviewed create flow rather than an immediate copy mutation. The browser pre-fills an editable draft with the persisted work definition, resets workflow status, and explicitly excludes comments, children, evidence, attachments, activity, and watchers. The create API accepts an optional active same-project `cloneSourceTaskId`; task allocation, the symmetric source relationship, clone audit event, automation/webhook effects, and idempotency record commit in one transaction. Unsaved source drafts block duplication so persisted and uncommitted state cannot be mixed silently.
46. Failed-specification follow-up creation is a convergent project transaction. Concurrent calls for the same evaluation return one linked task even across actors or API replicas; task allocation, exact evidence links, generic and origin audit events, automation and notification effects, and the webhook outbox commit atomically. The record UI uses a tooltip-labelled icon and replaces it with the stable created-task link after success rather than leaving an ambiguous repeat action.
47. Manual measurement entry keeps the chosen measurement field and its allowed unit as one controlled draft. Changing fields immediately selects that field's first allowed unit, successful continuous entry retains the valid field/unit pair, and the observation time starts at the user's current local minute and refreshes after each save. The browser never mutates unit options through raw HTML or lets form reset produce a field/unit mismatch.
48. Stable UUIDs for people, files, datasets, and task-linked engineering entities are API transport identifiers rather than normal form inputs. Configurable-record forms and grid editors use bounded searchable selectors with human-readable labels; new file choices are active and available, new dataset choices are active and ready, while an existing archived reference remains exactly resolvable for historical continuity. Ordinary task creation does not expose an entity-type/UUID pair: engineering evidence is attached from its originating record or evaluation flow, or through the task detail's reviewed link controls.
49. Record list and detail responses hydrate user, file-version, and dataset display labels in bounded page-level batches and return them separately from stable stored identifiers. Grid, Gallery, read-only, and detail surfaces consume the same label map, and single/multi-select reads resolve stored keys through their schema labels, so a visible page never issues one lookup per cell or exposes transport values as normal content. UUIDs and unknown option keys are only fallbacks for unavailable historical references. Controls must not advertise filter or sort operations that the typed query contract cannot execute; project-resource and structured-data filter/sort support remains hidden until it has explicit indexed semantics.
50. Task evidence reads resolve every supported engineering link in the same bounded task-detail query. Records use their display name and table identity, datasets use their name and immutable type/status, measurements use the record/field/value context, and specification evaluations use the record/field/outcome context; files and external sources retain their versioned metadata. The task browser never presents a UUID as ordinary evidence content, direct record evidence links open the exact record, archived evidence remains named and visibly archived, and unlink audit events preserve the resolved title so the activity trail stays understandable after removal.
51. Record editing surfaces prioritize domain labels and requiredness over implementation metadata. Every configurable field keeps an explicit required or optional marker, while its internal type is represented by a compact accessible icon with a localized hover/focus description. Stable record UUIDs remain available for API use, support, and traceability through a tooltip-labelled copy action and exact record routes, but are not ordinary heading content in either quick view or full detail.
52. The table-local API quickstart covers both data and metadata discovery. A scoped integration can copy stable workspace, project, and table identifiers, fetch the table's typed field contract, and then query, create, bulk-create, bulk-update selected fields, or export records without reverse-engineering the browser. Every generated snippet uses the public table identifier, bearer-token authentication, and idempotency where the mutation contract requires it; full OpenAPI documentation remains the authoritative reference.
53. Data-grid headers reserve scarce horizontal space for domain field names. Field types use the same compact localized icon vocabulary as record forms, with an accessible name and hover description, rather than repeating long type labels inside every column. Sorting, reordering, resizing, and context-menu semantics remain attached to the named column and do not depend on recognizing an icon.
54. Task-specific REST paths accept either the internal UUID or the immutable, human-readable project task key. Key lookup is case-insensitive and resolved only inside the already authorized project scope; missing and out-of-scope keys share the same not-found response. Responses continue to expose both identifiers, while relationship and hierarchy body fields retain explicit UUID references so compound mutations remain unambiguous.
55. Browser task deep links use the immutable human-readable task key as their canonical identifier. New links from boards, My work, workspace search, key dates, record evidence, automation history, and notifications use `?task=PROJECT-42`; copying a task link does the same. Existing UUID bookmarks remain compatible through the task API and are replaced with the canonical key after resolution, preserving saved filters and other bounded query state.
56. The task detail drawer is also a bounded review queue over the currently loaded board, list, or calendar results. Previous/next icon actions and `Alt+Up` / `Alt+Down` move through that exact filtered order while preserving query and saved-filter state. Direct deep links outside the loaded result page do not invent neighbors, and every navigation attempt uses the ordinary unsaved-draft guard before changing the canonical task-key URL.
57. The programmable schema catalog is a searchable, bounded Meta API rather than an all-project payload. It returns at most 100 tables per page with exact `pageInfo`, and hydrates complete ordered field metadata only for tables in that page. Integrations follow `hasNext`, restore one table through its exact endpoint, and may use the same literal search contract as the interactive table catalog; a large workspace can no longer make one schema-discovery request grow without bound.
58. Task archive and restore are version-checked lifecycle mutations, not unconditional state toggles. Both browser and programmable clients send the last-read task `rowVersion`; a concurrent edit, archive, or restore returns `TASK_VERSION_CONFLICT` without changing task state, audit history, notifications, automation state, or webhook outbox. Successful events record the exact version transition, archive reason when applicable, and return the incremented task representation; restore returns HTTP 200 because it mutates an existing task.
59. Task version conflicts are an explicit review state in the browser, not a raw repository error or blind retry. List-level moves, bulk changes, archive, and restore reload current server state and explain that another change won. A conflicting task-detail save fetches the latest representation separately while preserving the user's visible field draft; replacing that draft requires an explicit confirmation. The client never substitutes the new row version into an old mutation body, because doing so would silently overwrite the concurrent edit that optimistic concurrency was designed to protect.
60. Task provenance is visible in the ordinary detail context rather than discoverable only through an administrator's audit log. Task list and detail API representations document both the stable creator ID and the creator's current display name together with immutable creation and latest-update timestamps; the detail header renders those values in the viewer's locale. The first status-history entry is labelled as task creation, while later entries are labelled as status moves, so an initial state is never mistaken for an unexplained transition. Every status, field, evidence-link, comment, and comment-revision activity timestamp includes local date and time so same-day actions remain chronologically distinguishable.
61. The core platform REST surface is a generated-client contract rather than an undocumented browser implementation detail. Setup, local authentication, invitation and password-reset flows, workspace and project mutations, member-role changes, and member-group management publish strict Zod-derived request schemas and typed success representations in OpenAPI. The same input objects perform runtime validation, preventing the accepted payload from drifting away from the documented required fields, enums, bounds, and patterns. Contract tests enumerate these operations so a newly blank request or success schema fails the quality gate.
62. Engrove's engineering-specific API is held to the same generated-client standard as its no-code record API. The versioned unit registry, append-only measurements and corrections, specification revisions and evaluations, checksum-verified file upload lifecycle, immutable dataset processing, signed retrieval, retries, and storage cleanup publish typed success representations. Mutations with JSON bodies reuse strict Zod inputs for runtime validation and OpenAPI, with practical examples and explicit decimal, unit, timestamp, checksum, size, lifecycle, and source constraints. Contract tests enumerate every success representation in the engineering-types and file/dataset controllers so the product's differentiating traceability surface cannot regress into browser-only knowledge.
63. Engrove's reporting, external-evidence, and record-review APIs are first-class integration surfaces rather than browser-private helpers. Versioned chart and dashboard mutations publish their discriminated configuration and layout contracts; external sources publish HTTP(S)-only metadata, lifecycle, and optimistic-concurrency representations; record reviews publish strict discussion, reply, decision, and resolution contracts with complete current-thread responses. Bodyless restore and resolve actions remain bodyless. Contract tests enumerate every success representation and every JSON mutation body across these controllers so generated clients can support reproducible reporting and Jira-style review collaboration without reverse-engineering the UI.
64. Every HTTP operation in the published OpenAPI document declares its actual successful transport representation. Notification policy and acknowledgement, task automation, workflow lifecycle, saved-filter deletion, key-date lifecycle, onboarding, pilot feedback, and demo installation responses are typed; JSON mutations reuse strict runtime Zod inputs with practical examples. Demo installation has one retry-independent camel-case response. Health checks publish JSON status, Prometheus metrics publish `text/plain`, and OIDC navigation publishes 302 responses with `Location` rather than an empty default 200. Contract tests distinguish bodyless commands and redirects from missing JSON contracts, so zero blank 2xx representations is an enforced release invariant.
65. Task multi-selection is one view-independent triage state across Kanban and List. Switching between those views preserves the selected task set and exposes the same bulk-change toolbar; entering Calendar exits selection because its date-cell interaction model is different. List rows use checkbox semantics while selection is active, show their checked state visually, and support both pointer selection and Enter/Space keyboard toggling. Outside selection mode, the rows retain their ordinary open-detail behavior and `j`/`k` queue navigation.
66. Task selection supports contiguous range selection with Shift plus pointer click, Enter, or Space in both List and Kanban. The first ordinary selection becomes the anchor; a range adds every task between the anchor and target in the current rendered order, and clearing or leaving selection resets that anchor. Grouped-list ranges exclude collapsed groups because hidden rows must never be changed implicitly. The bulk toolbar exposes this shortcut without adding another persistent control, preserving the board's information density.
67. Authenticated CSV export follows the current table view boundary rather than silently exporting unrelated data. The browser sends visible field keys plus the active filters, search, sorts, project context, and archive state to a scoped POST route; the repository exhausts the complete matching result through bounded pages while excluding hidden fields. The original GET route remains a stable all-active-record compatibility contract sorted by display name. Audit events record only row count, field count, archive scope, and whether narrowing was applied, never the search or filter values themselves.
68. Interactive and new API CSV exports are private asynchronous jobs rather than request-sized in-memory responses. Creation is idempotent, the worker rechecks the requesting member's current export permission, streams bounded record pages into an attempt-specific object, and exposes only a short-lived signed download to that user. Successful artifacts expire and are physically removed after six hours, active upload checkpoints are protected from orphan cleanup, failed-attempt objects become cleanup candidates, and exports above 1,000,000 matching records are rejected before processing. The synchronous scoped and legacy all-active routes remain compatibility contracts.
69. Selected-record editing uses explicit field operations rather than replacing the view-visible record shape. One atomic request applies up to 20 unique set/clear operations to at most 100 records, preserves every unmentioned or hidden value, rejects calculated and append-only fields, validates engineering-reference UUID arrays, and routes each record through the ordinary projection, audit, webhook, and evaluation path. A missing, invalid, or stale record rolls back the complete batch; the browser caps selection at the same contract limit, confirms intent, and reloads authoritative state after a version conflict.
70. Task time tracking separates the original estimate, remaining estimate, and derived time spent. Time spent is never typed directly: it is the sum of active authored work entries. Creating, editing, or soft-deleting a work entry and adjusting remaining time commit in one task-version-checked transaction with immutable audit and webhook effects. Automatic adjustment subtracts a new duration, applies only the edited-duration delta, and restores a deleted duration; callers may instead preserve or explicitly set remaining time. Contributors and Engineers may log work, while only the author or an administrator may edit or delete an entry. The task drawer keeps estimates, progress, and bounded work history in context without reintroducing status controls on cards.
71. Configurable-table permissions are a server-enforced narrowing layer over global roles and API-token scopes. Each table independently controls visibility, create, update, and archive/restore through role thresholds, specific active members or member groups, or nobody. Hidden tables are excluded from browser, table, and schema catalogs and return the same not-found result as an absent table; Owner and Admin retain visibility only to repair policy. Every record mutation path, including bulk, CSV, undo, archive, restore, API tokens, and public Forms, evaluates the same repository policy. Complete policy replacement uses its own optimistic row version, validates normalized organization subjects, preserves a conflicting browser draft for explicit reload, and emits an immutable audit event without exposing policy subjects in ordinary table responses.
72. Record CSV import is a reviewable data-onboarding workflow rather than an immediate file-write action. Preview performs bounded parsing without mutation, shows representative rows, and suggests only unambiguous key-or-name mappings. The caller explicitly maps or ignores each source column and maps one column to record name. Duplicate handling is an explicit create-all, skip-match, or update-match choice; matching is available only through a mapped supported unique field, and update preserves every unmapped value while enforcing both global and table-specific update permission. Human-friendly relation names and member emails are resolved only when exactly one active target exists. Import remains idempotent, isolates row failures with savepoints, bounds returned error details, records aggregate audit metadata, and never silently populates append-only, binary-reference, or calculated fields.
73. Organization roles describe capabilities, not universal discovery. Workspaces may narrow organization visibility to selected active members or groups, and projects may narrow inherited workspace visibility again without ever widening the parent boundary. Owners, administrators, and the creator retain recovery access. Stable SQL predicates protect direct routes, catalogs, navigation, search, workspace overview, My Work, key dates, task projections, due-date notifications, and API-token traffic; hidden resources return not found. Create-time visibility is atomic, policy replacement has independent optimistic concurrency, restriction prunes unauthorized watchers and notifications, and administrators manage both levels from compact inline settings.
74. Release accessibility is an automated product invariant rather than a visual-review claim. Browser E2E runs axe against the workspace overview, configurable-data grid, task board, record quick view, and task detail in both light and dark themes using WCAG 2.0, 2.1, and 2.2 A/AA rules. Shared controls keep AA text contrast and at least 24-by-24-pixel pointer targets without increasing the surrounding workspace whitespace.
75. API responses carry defense-in-depth browser headers at the origin, including MIME-sniffing prevention, frame denial, a no-referrer policy, and disabled camera, geolocation, and microphone capabilities. Production also emits one-year HSTS while the TLS edge remains responsible for preserving or strengthening it. Authenticated reads remain privately revalidated, all versioned API mutations and secret-bearing reads are `no-store`, and health, metrics, and generated API documentation cannot be stored by intermediary or browser caches.
76. The programmable data surface includes a dependency-free, publish-ready TypeScript client rather than relying only on generated snippets. It exposes stable API errors, request IDs, ETags, quota metadata, typed table operations, and bounded async page iteration. The client rejects non-API and cross-origin request paths before attaching a bearer token, retries only reads or explicitly idempotent writes, and preserves one idempotency key and body across every retry attempt. The table-local quickstart exposes SDK examples next to cURL and raw Fetch examples, while OpenAPI remains the complete contract.
77. Task rank is one project-wide planning order with pointer, keyboard, and context-menu access rather than a drag-only board detail. `Alt+Up` and `Alt+Down` move a focused task relative to loaded peers, while the context menu moves it to the absolute top or bottom of its workflow status even when filters or pagination hide other tasks. `Alt+Left` and `Alt+Right` retain workflow transition behavior. Every rank mutation remains version-checked, serialized by the project advisory lock, audited separately from status changes, and announced as reordering rather than a misleading status move.
78. The TypeScript SDK treats project work as a first-class integration surface rather than stopping at configurable records. A project-scoped task client supports bounded list iteration, UUID-or-key lookup, idempotent creation, version-checked editing and ranking, atomic bulk changes, comments, watching, archive, and restore. Task response types retain the stable OpenAPI JSON field names so SDK values, webhook fixtures, and raw REST payloads remain interchangeable. Automatic retries remain limited to reads and task creation carrying one preserved idempotency key; stale versioned mutations require an explicit read-and-review cycle.
79. Destructive confirmation and short-text collection use one application action-dialog contract rather than browser-native `confirm` and `prompt` surfaces. Requests queue instead of replacing one another; destructive actions are visually distinct; confirmation starts on the safe cancel action; text prompts expose a real field label and required state; and every dialog traps focus, closes with Escape or backdrop intent, restores the triggering focus, and uses the active locale. A modal stack lets only the topmost nested dialog consume keyboard dismissal. Unsaved task and saved-view navigation keeps a synchronous no-draft path so ordinary selection remains immediate while guarded transitions await an explicit result.
...
`RecordView.config` contains field IDs in display order, per-field pixel widths, validated record filters and sorts, row density, page size, up to three optional scalar-field grouping levels, and optional type-compatible field summaries. Every referenced field must belong to the same object type. Active view names are case-insensitively unique within an object type. Updates and archive operations require the last-read `rowVersion`; a stale write returns `VERSION_CONFLICT`. Views are shared project configuration rather than private browser preferences, and all mutations write audit events.
...
- direct cell editing for mutable record names and properties, searchable display-name relation pickers, and exact file or dataset references,
...
- page-scoped row selection with permission-aware bulk field editing and archive,
...
- user can select up to 100 visible rows and atomically set or clear several editable fields without replacing hidden values,
